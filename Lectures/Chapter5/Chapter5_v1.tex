...
\begin{frame}{Evaluating OVB}
	...
	\begin{wideitemize}
		...
		\pause
		\item
		Hence $\tilde\beta_D$ will be very biased for $\beta_D$ if the omitted variable $X_{i1}$ is both highly correlated with $Y_i$ and highly correlated with $D_i$\smallskip\pause{}
		\begin{itemize}
...
\begin{frame}
	\begin{wideitemize}
		...
		\begin{tabular}{lrr}
			Variable & Coefficient & SE \\ \hline
			Private school ($\tilde\beta_D$) & 0.135 & 0.055 \\
			Constant & [...] & [...]
		\end{tabular}
...
\begin{frame}{Omitted Variable Bias Formula - Multiple Variables}
	...
	\pause
	\item
	But since we don't observe $X_{i1}$, we instead estimate the regression $Y_i = \tilde\beta_0 + \tilde\beta_D D_i + \tilde\beta_1 X_{i1} + \epsilon_i$
	
	\pause
	\item
	How does $\tilde\beta_D$ relate to $\beta_D$? \pause Answer: $$\tilde\beta_D= \beta_D+ \beta_2 \gamma_D$$ 
	
	\noindent where $\gamma_D$ is the coefficient from the regression $$X_{i2} = \gamma_0 + \gamma_D D_i + \gamma_1 X_{i1} + u_i$$ 

	\item This is similar to the OVB formula we had from before, except everything controls for $X_{i1}$
\end{wideitemize}
...
\begin{frame}{More Evidence - Mountjoy and Hickman (2021)}
	\begin{itemize}
		\item Mountjoy and Hickman do a modern version of Dale and Krueger using data on all 4-year public universities in Texas
...
\begin{frame}{What are the higher value-added colleges doing}
...
\begin{frame}{Chetty, Deming, Friedman (2025)} 
	...
	\begin{wideitemize}
		...
		\end{itemize}	
		\item
		Like Dale and Krueger, they don't find much effect on \emph{average} earnings
		
		\item
		But they do find impacts on ``elite'' outcomes --- earnings in top 1\%, working at ``elite firm'', attending ``elite'' gradaute school
	\end{wideitemize}
...
\begin{wideitemize}
	...
	\pause
	\item
	I simulate this fake treament 1000 times, and estimate 
...
\begin{frame}
	...
	\only<2>{	
		\begin{wideitemizeshort}
			\item When testing the pooled effect across all states, we find a significant effect 6\% of the time
		\end{wideitemizeshort}
		}
	
		\only<4>{	
		\begin{wideitemizeshort}
			\item When testing the pooled effect across all states, we find a significant effect 41\% of the time
		\end{wideitemizeshort}
	}
	...
	\only<6>{	
		\begin{wideitemizeshort}
			\item When testing the pooled effect across all states, we find a significant effect 93\% of the time
		\end{wideitemizeshort}
	}
...
\begin{wideitemize}
	...
	\item
	Then 
	$$P( N_{sig} > 0  ) \leq \sum_k P( \text{k is significant}  ) = K (0.05/K) = 0.05 $$
...
\begin{frame}
	\begin{wideitemize}
		\item
		Let $\hat{t}_k$ be the t-statistic for null hypothesis $k$. Under null hypothesis $k$, we have that $\hat{t}_k \to_d \pause N(0,1)$.
		
		\item
		If the samples for each hypothesis are independent, then if all $K$ null hypotheses are satisfied, we have $(\hat{t}_1,\ldots,\hat{t}_K)' \to_d N(0, \mathbf{I})$
		
		\item
		Consider the \emph{$F$-statistic}: $F = \sum_k \hat{t}_k^2$. \pause By the continuous mapping theorem, if all $K$ null hypotheses are satisifed
		
		$$F \to_d  \sum_k Z_k^2 \text{ for } Z \sim N(0,\mathbf{I})$$
		This is called a \emph{chi-squared} ($\chi^2$) distribution with $K$ degrees of freedom 
		...
		\item
		This approach is called an $F$\emph{-test}.
...
\begin{frame}
	Probability of rejecting the joint null that the treatment effect is zero for all states \\ \vspace{1cm}
	\begin{tabular}{lrr}
		Simulation & Individual tests & Joint test \\ \hline
		Pooled & 0.06 & 0.06 \\
		10 States & 0.41 &  0.04 \\
		50 States & 0.93 & 0.05
	\end{tabular}
...
\begin{frame}{$F$-tests can have better power than Bonferroni}
	\begin{wideitemize}
		\item
...
\begin{frame}{Testing multiple regression coefficients}
...
\begin{wideitemize}
	...
	\item
	We showed in earlier classes that  $\sqrt{N}(\hat\beta - \beta_0) \to_d N(0,\boldsymbol{\Sigma})$. To test the null $H_0: \beta = \beta_0$, we then use  $F = \sqrt{N} \hat{\boldsymbol{\Sigma}}^{-\frac{1}{2}} (\hat\beta - \beta_0)$ 
	
	\item
	This is implemented with the $\texttt{test}$ function in Stata. E.g., $\texttt{test age agesq}$ tests the null that the coefficients on age and age-squared are both zero (so wages don't depend on age)
\end{wideitemize}	
