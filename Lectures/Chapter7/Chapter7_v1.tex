...
\begin{frame}{Martin and Yurukoglu (2017)}
	...
	\begin{wideitemize}
		...
		\item
		We could compare places where people watch more Fox News to places where people watch less Fox News. Would that give us a causal effect? \pause
		 	\begin{itemize}
		 		\item 
	 	...
	 	\item
	 	We need an instrument that is as good as randomly assigned and affects voting patterns only through its impact on Fox News viewership. Any ideas? \pause
	 	
	 	\item
	 	Martin and Yurukoglu propose to use the position of Fox News in the cable channel lineup as an IV
	 		\begin{itemize}
	 			\item 
...
\begin{frame}{Martin and Yurukoglu's 2SLS Specification}
	First stage: 
	$$D_{i} = \pi Z_i + \boldsymbol{\gamma}' \boldsymbol{X}_i + u_i $$
	where $D_{i}$ is average minutes of Fox News watched per week in Zip code $i$, $\boldsymbol{X}_i$ is a vector of control variables including state or county FEs, the position of MSNBC, and some demographic variables (income, age, race, etc) \medskip
	
	Second stage:
	$$Y_{i} = \beta \hat{D}_i + \boldsymbol{\delta}' \boldsymbol{X}_i + \epsilon_i $$
	
	\noindent where $Y_{i}$ is Republican vote share in the 2008 election and $\hat{D}_i$ is the prediction from the first-stage
\end{frame}

\begin{frame}{Evaluating the assumptions}
	\begin{wideitemize}
		\item
		\textbf{Independence:} \pause Need that Fox News position is as good as random (conditional on observable characteristics). \pause This is the most tenuous: seems plausible, but worry that Fox News might be put earlier in places with higher demand
		
		\item
		\textbf{Exclusion:} \pause Need that Fox News position impacts voting only through Fox News viewership. \pause Seems relatively plausible
		
		\item
		\textbf{Relevance:} \pause Need that cable position impacts Fox News viewership. \pause We'll see that it does
		
		\item
		\textbf{Monotonicity:} \pause Need that being in a lower cable position only increases viewership. \pause Seems plausible, although could be violated if being later increases proximity to other popular channels
	\end{wideitemize}
\end{frame}

\begin{frame}{First Stage}
		\includegraphics[width = 0.9 \linewidth]{my-first-stage}
...
\begin{frame}{Some evidence on independence}
...
\begin{frame}{Validation using satellite viewers}
