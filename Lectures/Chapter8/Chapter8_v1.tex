...
\begin{frame}{When does Fuzzy RD Give a LATE?}
	\begin{wideitemize}
		...
		\pause
		\item
		\textbf{Continuity}. Need that $E[Y_i(d) | R_i]$ is continuous at the cutoff for $d=0,1$, as is share of compliers/always-takers/never-takers
...
\begin{frame}{Zimmerman (2009)}
	\begin{wideitemize}
		\item
		Zimmerman (2009) studies the earnings impact of attending a 4-year state university
		
		\item
		Focuses on Florida International University (FIU), the least selective of the public 4-year universities in FL 
		
		\item
		State law mandated that students at public universities have a GPA of at least 3.0, but schools had discretion in how they \emph{calculated} GPA. FIU was the most lenient
		
		\item
		Hence, students near the threshold for FIU were typically ineligible at any other 4-year public university in FL
			\begin{itemize}
				\item 
				Students who didn't get in may attend community college or private college 
			\end{itemize}
		
		\item
		Zimmerman conducts a fuzzy RDD design using the 3.0 GPA cutoff (using FIU's calculation)
	\end{wideitemize}
\end{frame}
...
\begin{frame}
	...
	One concern: there is some bunching just above the cutoff. Do more motivated students manage to get \emph{exactly} the GPA they need? \medskip
	
	Alternatively, there are more combos of grades that lead to 3.0 than 2.99
\end{frame}
...
\begin{frame}
	...
	However, there doesn't appear to be a discontinuity in observable characteristics around the cutoff
\end{frame}
...
\begin{frame}
	...
	\begin{wideitemize}
		\item
		Note the first-stage on attending FIU is about half the first-stage on admission.
		
		\item
		This suggests a LATE for attendance of  \$3-4K (about half of mean earnings below cutoff)
...
\begin{frame}{Discussion}
	\begin{wideitemize}
		\item
		The results suggests large returns to attendance of FIU 
		
		\item
		Key interpretation Q: what is the alternative? 
		
		\item
		The paper shows a reduction in community college attendance; students may also be pulled from private universities 
		
		\item
		Sharp contrast between the large returns here to bottom-tier 4-year public vs Dale \& Krueger, who find small gains to \emph{extremely} selective vs \emph{still selective} colleges. 
	\end{wideitemize}
\end{frame}
